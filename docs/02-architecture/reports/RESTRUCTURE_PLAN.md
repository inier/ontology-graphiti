...
> **移动记录**: 2026-05-03 从 docs/ 移动到 docs/architecture/
> **作者**: 软件架构师
> **说明**: 本文档记录历史目录重构方案，当前项目结构已按 ADR-033 执行
...
```
┌────────────────────────────────────────────────────┐
│  L4 业务领域层 (biz/)  — 领域分析、模拟器、Agent     │  ← 对外 API 暴露层
├────────────────────────────────────────────────────┤
...
│   │   │   │   ├── domain.py      # ← ontology/domain_ontology.py
...
│   │   │
│   │   └── mock_data/              # 【Mock 数据生成模块】
│   │       ├── __init__.py
│   │       └── data_generator.py   # ← data/simulation_data.py
│   │                               # 职责: 为构建本体场景生成 Mock 样本数据
│   │                               # 注意: 与 simulator/ 的"推演验证"完全不同！
│   │                               #   - mock_data: 数据准备阶段（填充本体）
│   │                               #   - simulator: 推演验证阶段（辅助决策）
│   │
│   ├── workspace/                  # 【工作空间管理模块】
...
│   │       │                           # 能力: create_scenario(), run_simulation(), pause(), resume()
│   │       │
...
│   └── storage/                        # 【数据存储层】
│       ├── __init__.py
│       ├── scenarios/                  # ← ontology/versions/scenarios/ 迁移（推演场景）
│       ├── versions/                   # ← ontology/versions/*.json 迁移（本体版本）
│       ├── states/                     # Agent 状态持久化
│       └── exports/                    # 导出文件
│
# ═══════════════════════════════════════
...
class SimulatorEngine:
    """推演引擎 - What-if Analysis / Dry-run for Decision Support"""
...
# 逐一 re-export
from odap.infra.graph.graph_service import DomainGraphManager
