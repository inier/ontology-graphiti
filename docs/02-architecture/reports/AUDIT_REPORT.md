...
> **用途**: 文档审计报告（历史参考）
> **上游文档**: 00-requirements/req-ok.md, ARCHITECTURE.md
...
- **详细分析**: 
  - `do../07-adr/README.md`: ADR-014 = "可扩展图表系统"
...
- **问题**: ARCHITECTURE.md 第 18.1 章需求追溯矩阵中 ADR 编号引用存在偏移
  ...
  - 矩阵引用 `ADR-020`=战争实体 → 实际应为 ADR-036
  - 矩阵引用 `ADR-036`=模拟数仓 → 实际应为 ADR-022
...
14. `do../07-adr/ADR-026_mcp_protocol_integration.md`（移除断链）
