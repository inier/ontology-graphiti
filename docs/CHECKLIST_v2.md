...
- [x] WR-01.D1: 双时态查询 API 设计完成（valid_time + transaction_time 参数签名）
- [x] WR-01.D2: 混合检索策略文档完成（向量→关键词→内存降级规则）
- [x] WR-01.D3: Episode 批量写入接口设计完成
- [x] WR-01.D4: 连接池 + 断路器配置参数确定

#### 实现

- [x] WR-01.I1: `query_temporal()` 方法实现（valid_time/transaction_time 过滤）
- [x] WR-01.I2: `search_hybrid()` 重构（向量权重 + 关键词权重可配置）
- [x] WR-01.I3: `add_episodes_batch()` 批量写入（去重 + 幂等）
- [x] WR-01.I4: 连接池（max_size=20, timeout=30s, idle_timeout=300s）
- [x] WR-01.I5: 断路器（failure_threshold=5, recovery_timeout=60s）
- [x] WR-01.I6: PerformanceMonitor（查询耗时、缓存命中率、连接池使用率）
...
- [x] WR-02.D1: 合并方案设计（permission_checker → opa_policy 接口映射）
- [x] WR-02.D2: ABAC 策略模型设计（主体属性 + 资源属性 + 环境属性 + 操作）
- [x] WR-02.D3: Bundle 热更新协议设计（增量 vs 全量）
- [x] WR-02.D4: 策略沙箱接口设计（What-If 输入/输出格式）

#### 实现

- [x] WR-02.I1: permission_checker 代码合并到 opa_policy 模块
- [x] WR-02.I2: ABAC Rego 策略 v2（支持属性条件表达式）
- [x] WR-02.I3: Bundle 管理器（策略文件打包 + 版本 + 热推送）
- [x] WR-02.I4: PolicySandbox（不修改真实策略，模拟决策）
- [x] WR-02.I5: BatchEvaluator（批量权限检查 + 结果缓存）
- [x] WR-02.I6: 旧接口兼容层（permission_checker 的公共 API 保持可用）
