...
|------|------|------|---------|---------|--------|------|
| BE-001 | odap/gateway/api_gateway_v2.py | ServiceProxy.forward | 返回硬编码 `{"status": "ok"}`，forward_ws/forward_sse 均为空实现 | 实现真实代理逻辑或移除 | P1 | ⬜ 待处理 |
| BE-002 | odap/gateway/api_gateway_v2.py | AuthHandler.logout | 方法体为空（pass） | 实现 logout 逻辑 | P1 | ⬜ 待处理 |
| BE-003 | odap/gateway/api_gateway_v2.py | RateLimitType.SLIDING_WINDOW | 枚举值定义但从未使用 | 移除 | P2 | ⬜ 待处理 |
| BE-004 | odap/biz/frontend_compat/api/routes.py | log_error / _log_error_async | 函数体为空（pass），错误日志全部丢失 | 实现错误日志记录 | P0 | ⬜ 待处理 |
| BE-005 | odap/biz/frontend_compat/api/routes.py | update_scenario | 只查询不更新，直接返回原数据 | 实现更新逻辑 | P0 | ⬜ 待处理 |
| BE-006 | odap/biz/frontend_compat/api/routes.py | delete_scenario | 只检查存在但不删除，返回假成功 | 实现删除逻辑 | P0 | ⬜ 待处理 |
...
| BE-009 | odap/biz/frontend_compat/api/routes.py | get_version | 返回硬编码数据 | 对接真实数据源 | P1 | ⬜ 待处理 |
| BE-010 | odap/biz/frontend_compat/api/routes.py | rollback | 只返回成功但不执行回滚 | 实现回滚逻辑 | P0 | ⬜ 待处理 |
| BE-011 | odap/biz/frontend_compat/api/routes.py | diff_versions | 返回空 changes | 实现对比逻辑 | P1 | ⬜ 待处理 |
| BE-012 | odap/biz/frontend_compat/api/routes.py | query_relations | 永远返回空列表 | 实现查询逻辑 | P1 | ⬜ 待处理 |
| BE-013 | odap/biz/frontend_compat/api/routes.py | get_entity_history | 永远返回空列表 | 实现历史查询 | P1 | ⬜ 待处理 |
| BE-014 | odap/biz/frontend_compat/api/routes.py | get_graph_progress/cancel/history/detail | 均返回硬编码/空数据 | 实现或移除 | P1 | ⬜ 待处理 |
| BE-015 | odap/biz/frontend_compat/api/routes.py | get_topic_stats | 返回硬编码模拟数据 | 对接真实统计 | P1 | ⬜ 待处理 |
| BE-016 | odap/biz/frontend_compat/api/routes.py | get_query_history | 永远返回空列表 | 实现查询历史 | P2 | ⬜ 待处理 |
...
| BE-026 | odap/web/api/app.py | _ensure_initial_version | 异常被静默吞掉（except Exception: pass） | 添加日志记录 | P1 | ⬜ 待处理 |
...
| BE-029 | odap/biz/business/storage/sqlite_storage.py | delete_* 方法 | 使用 conn.total_changes 判断删除成功 | 改用 cursor.rowcount | P1 | ⬜ 待处理 |
...
| BE-033 | odap/infra/security/audit_logger.py | log_sync | asyncio.run() 在已有事件循环中会抛异常 | 使用 nest_asyncio 或检测事件循环 | P1 | ⬜ 待处理 |
| BE-034 | app/main.py | @app.on_event("startup") | FastAPI 已弃用此方式 | 改用 lifespan | P2 | ⬜ 待处理 |
| BE-035 | odap/biz/frontend_compat/api/routes.py | ingest_* | Celery 不可用时仍返回 task_id | 检查可用性后再返回 | P1 | ⬜ 待处理 |
...
| 编号 | 任务 | 预计影响 | 状态 |
|------|------|---------|------|
| BE-004 | 实现 log_error 错误日志记录 | 日志系统 | ⬜ 待处理 |
| BE-005 | 实现 update_scenario 更新逻辑 | 场景管理 | ⬜ 待处理 |
| BE-006 | 实现 delete_scenario 删除逻辑 | 场景管理 | ⬜ 待处理 |
| BE-010 | 实现 rollback 回滚逻辑 | 版本管理 | ⬜ 待处理 |
...
| BE-029 | 修复删除判断逻辑 | 数据正确性 | ⬜ 待处理 |
| BE-030 | 消除循环依赖 | 代码规范 | ⬜ 待处理 |
| BE-035 | 修复假 task_id 返回 | 数据正确性 | ⬜ 待处理 |
...
| 编号 | 任务 | 预计影响 | 状态 |
|------|------|---------|------|
| BE-003 | 移除未使用枚举值 | 代码整洁 | ⬜ 待处理 |
| BE-024 | 合并冗余 Agent 端点 | API 简化 | ⬜ 待处理 |
| BE-025-027 | 修复小问题 | 代码规范 | ⬜ 待处理 |
| BE-031-032 | 修复异常处理 | 健壮性 | ⬜ 待处理 |
| BE-034 | 迁移到 lifespan | 框架兼容 | ⬜ 待处理 |
...
| FE-018 | 修复 `handleDeleteSession` 中 `sessionId === sessionId` bug，改为 `deletedSessionId === sessionId` | 2026-05-18 |
