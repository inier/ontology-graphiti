...
MCP（Model Context Protocol）协议集成模块是Graphiti系统与外部战场仿真系统（战场仿真器、雷达模拟器、气象数据源）的标准接口层。基于MCP v1.0标准，为OpenHarness Agent提供统一的第三方服务接入能力，支持**协议标准化、扩展性、安全性**的战场数据集成。
...
class MCPServerConfig:
    """MCP Server配置"""
    name: str                      # 服务器名称
    server_type: str               # 服务器类型 (battlefield_simulator, radar_simulator, weather_source)
...
#### 3.2.1 战场仿真器MCP Server
```python
# external/battlefield_simulator/mcp_server.py
...
class BattlefieldSimulatorMCPServer:
    """战场仿真器MCP Server实现"""
    
    def __init__(self):
        self.server = Server("battlefield-simulator")
    ...
    async def list_tools(self) -> List[types.Tool]:
        ...
        return [
            types.Tool(
                name="battlefield.get_scenario",
                description="获取战场场景信息",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "scenario_id": {
            ...
            types.Tool(
                name="battlefield.update_unit_position",
                description="更新单位位置",
                inputSchema={
                    "type": "object",
                    "properties": {
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> List[types.TextContent]:
        """调用战场仿真工具"""
        if name == "battlefield.get_scenario":
            return await self._get_scenario(arguments["scenario_id"])
        elif name == "battlefield.simulate_attack":
    ...
    async def list_resources(self) -> List[types.Resource]:
        ...
        return [
            types.Resource(
                uri="battlefield://scenarios",
                name="战场场景列表",
                description="所有可用的战场仿真场景",
                mimeType="application/json"
            ),
            types.Resource(
                uri="battlefield://units/status",
                name="单位状态",
                description="所有作战单位的实时状态",
                mimeType="application/json"
            ),
            types.Resource(
                uri="battlefield://terrain/map",
                name="地形地图",
                description="战场地形高程图",
                mimeType="image/png"
            )
...
services:
  ...
  battlefield-simulator:
    build:
      context: ./external/battlefield_simulator
      dockerfile: Dockerfile
    container_name: battlefield-simulator-mcp
    environment:
...
mcp:
  ...
  servers:
    battlefield_simulator:
      name: "battlefield-simulator"
      type: "battlefield_simulator"
      transport: "websocket"
      endpoint: "ws://battlefield-simulator:9000"
      capabilities:
        - "scenario_management"
        - "unit_simulation"
        - "combat_outcome"
...
test_coverage:
  ...
  e2e_tests:
    - battlefield_simulator_scenario: 70%
...
#### 8.2.2 集成测试
```python
# tests/mcp/integration/test_battlefield_simulator.py
import pytest
import json
from httpx import AsyncClient
from core.mcp.battlefield_simulator import BattlefieldSimulatorAdapter

class TestBattlefieldSimulatorIntegration:
    
    @pytest.fixture
    async def simulator_adapter(self):
        """创建战场仿真器适配器"""
        adapter = BattlefieldSimulatorAdapter("ws://localhost:9000")
...
ws.onmessage = (event) => {
  ...
  switch(data.type) {
    case 'radar_detection':
      ...
      break;
    case 'battlefield_event':
      handleBattlefieldEvent(data.payload);
      break;
    case 'weather_update':
...
async function simulateAttack() {
  try {
    const result = await executeToolViaWebSocket('battlefield.simulate_attack', {
      attacker_id: 'unit-001',
      target_id: 'unit-002',
      weapon_type: 'missile'
    });
...
- [OpenHarness集成指南](../openharness_bridge/DESIGN.md)
- [战场仿真器API文档](../../external/battlefield_simulator/README.md)
