...
## 更新摘要
**所做更改**
- 更新了前端界面导航结构，移除了策略管理界面中的审计日志标签页
- 新增了独立的审计菜单入口，审计日志功能现在通过专门的审计页面访问
- 完善了前端审计功能的页面组件和Store管理
- 更新了路由配置以支持新的审计页面访问方式
...
**更新** 策略管理界面中的审计日志标签页已被移除，审计日志功能现在通过独立的审计菜单入口访问，提高了界面的清晰度和用户体验。
...
**更新** 前端审计界面现在通过独立的路由入口访问，不再集成在策略管理页面中，提供了更好的用户体验和功能分离。
...
**更新** 前端审计页面优化了大数据量场景下的性能表现，通过虚拟滚动和分页加载提升用户体验。
...
**更新** 前端审计页面增加了错误处理和状态提示，便于用户理解系统状态和进行故障排查。
...
**更新** 前端界面重构后，审计功能通过独立的菜单入口访问，提供了更好的用户体验和功能分离。策略管理页面专注于策略编辑功能，审计日志功能更加专业化和易于访问。
...
**更新** 前端审计页面提供了多种视图模式，包括列表视图和时间线视图，满足不同场景下的审计分析需求。
...
**更新** 前端审计页面增加了性能优化配置，包括分页大小设置和虚拟滚动支持。
...
**更新** 审计功能现在更好地支持策略管理场景，提供完整的策略生命周期审计追踪。
