...
- 内置工具模块：分析、计算、可视化、智能体、Agent 图谱工具等
...
- 内置工具模块
  - 分析、计算、可视化、智能体、Agent 图谱工具等，按领域分类注册
...
## 结论
ODAP 工具技能系统通过“工具层 + 平台层 + 内置工具”的分层设计，实现了统一的工具注册、动态加载、权限控制、健康监控与工具链编排。技能系统支持热插拔、版本管理与生命周期管理，满足运行时配置与热生效需求。内置工具覆盖分析、计算、可视化、智能体与图谱操作等关键领域，便于快速扩展与插件集成。
