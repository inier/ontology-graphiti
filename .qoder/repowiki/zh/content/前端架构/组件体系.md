...
- [QAPanel.tsx](file://frontend/src/modules/shared/components/QAPanel.tsx)
- [QAChatPage.tsx](file://frontend/src/modules/qa/pages/QAChatPage.tsx)
- [i18nStore.ts](file://frontend/src/modules/shared/store/i18nStore.ts)
...
**更新** 本版本重点反映了应用布局组件的显著增强，包括历史跟踪、API管理、指南针导航、智能问答、全球语言支持等图标的功能改进，以及版本管理、国际化支持等新功能的集成。
...
P8["pages/QAChatPage.tsx"]
P9["pages/I18nAdminPage.tsx"]
end
A --> R
R --> L1
L1 --> T1
...
- 布局组件
  - AppLayout：提供统一的头部、侧边栏与内容区布局，作为页面级容器。现已增强支持历史跟踪、API管理、指南针导航、智能问答、全球语言支持等图标导航。
...
- 页面组件
  ...
  - QAChatPage：智能问答聊天页面，集成历史记录与LLM对话功能。
  - I18nAdminPage：国际化管理后台，支持翻译条目管理与LLM自动翻译。

**更新** 新增的版本历史管理、智能问答、国际化管理等页面体现了组件体系的扩展能力。
...
- 设计原则
  ...
  - **新增**：集成了历史跟踪、API管理、指南针导航、智能问答、全球语言支持等图标导航。
- Props接口
...
AppLayout --> VersionHistory : "导航"
AppLayout --> QAPanel : "导航"
AppLayout --> I18nAdminPage : "导航"
```

**更新** AppLayout现在承担着更多导航职责，集成了多个业务领域的图标导航。
...
### 页面组件：VersionHistory
- 设计原则
  - 专门的版本历史管理页面，提供版本对比、回滚、删除等操作。
- 功能特性
  - 支持版本列表展示、版本对比、回滚到指定版本、删除版本。
  - 集成实体数量、关系数量、事件数量等统计数据。
- 状态管理
  - 使用Zustand状态管理，维护版本列表与选择状态。
- 组合模式
  - 使用Ant Design的Table组件展示版本信息，结合Popconfirm确认框保证操作安全性。

**新增** 版本历史管理页面体现了ODAP平台强大的版本控制能力。
...
### 页面组件：QAPanel
- 设计原则
  - 智能问答面板组件，提供简洁的问答交互界面。
- 功能特性
  - 支持用户消息发送、AI助手回复、消息历史展示。
  - 集成消息源链接，支持从问答结果跳转到相关图谱节点。
- 状态管理
  - 使用React Hooks管理消息列表、输入状态与加载状态。
- 组合模式
  - 使用Ant Design的List组件展示消息历史，结合Card和Avatar组件美化界面。

**新增** QAPanel组件为智能问答功能提供了标准化的UI组件。
...
### 页面组件：I18nAdminPage
- 设计原则
  - 国际化管理后台页面，提供翻译条目管理与LLM自动翻译功能。
- 功能特性
  - 支持按模块和语言筛选翻译条目。
  - 提供在线编辑翻译、LLM自动翻译与人工审核流程。
- 状态管理
  - 使用Zustand状态管理，维护翻译列表、模块列表、语言列表等状态。
- 组合模式
  - 使用Ant Design的Select组件进行筛选，结合Table组件展示翻译条目。

**新增** 国际化管理页面体现了ODAP平台的全球化支持能力。
...
**更新** 新增的版本历史、智能问答、国际化管理等功能对性能提出了新的要求，需要特别关注大数据量的渲染优化。
...
- 调试建议
  ...
  - **新增**：对于版本历史页面，检查版本对比API的返回格式与数据完整性。

**章节来源**
- [FRONTEND_COMPONENT_DESIGN.md:658-674](file://docs/04-ui/FRONTEND_COMPONENT_DESIGN.md#L658-L674)

## 结论
ODAP前端组件体系以5级分层为核心，通过布局、模板、组织、分子与原子组件的有序组合，实现了高复用、强一致与易维护的前端架构。配合统一的命名规范、样式系统与响应式策略，开发者可以快速构建复杂业务页面。

**更新** 本版本的组件体系在原有基础上得到了显著增强，新增的历史跟踪、API管理、指南针导航、智能问答、全球语言支持等功能，体现了平台向更全面的本体驱动分析决策平台的发展方向。建议持续完善大型组件拆分、统一状态管理策略与测试覆盖，以提升长期可维护性与质量。
...
**更新** 新增了国际化存储管理的useI18nStore，支持多语言切换与翻译管理。
...
### 新增功能组件
- **版本历史管理**：提供完整的版本控制功能，包括版本对比、回滚、删除等操作。
- **智能问答**：集成LLM对话能力，支持历史记录与消息源追踪。
- **国际化管理**：提供翻译条目管理与LLM自动翻译功能。
- **图标导航增强**：AppLayout集成了历史跟踪、API管理、指南针导航、智能问答、全球语言支持等图标导航。

**新增** 这些新功能体现了ODAP平台从单一本体管理向综合性分析决策平台的演进。