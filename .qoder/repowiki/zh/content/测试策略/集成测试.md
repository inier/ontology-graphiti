...
- [openharness/tests/fixtures/fake_mcp_server.py](file://openharness/tests/fixtures/fake_mcp_server.py)
- [openharness/tests/test_mcp/test_integration.py](file://openharness/tests/test_mcp/test_integration.py)
...
- 外部服务集成测试：Neo4j图数据库、Redis缓存、OPA策略服务、MCP协议桥接
- 数据流测试：从数据摄入、本体构建到查询与可视化的端到端验证
- API集成测试：REST端点、参数校验、响应格式、错误码处理
- 数据库集成测试：本体数据一致性、查询性能、事务处理
- MCP协议与OpenHarness集成测试：连接、工具发现与调用
- 测试环境配置、Mock服务搭建、测试数据管理
- 执行策略、失败排查、性能基准测试

## 项目结构
ODAP平台采用FastAPI后端与React前端，集成OpenHarness Agent与MCP协议桥接，使用Docker Compose编排Neo4j、Redis、OPA等外部依赖。集成测试主要位于tests/integration目录，并通过docker-compose.test.yml启动受控测试环境。
...
## 核心组件
- FastAPI应用与路由注册：集中于odap/web/app.py，包含工作空间、角色、技能、事件模拟器、本体管理、QA、审计、OPA策略、MCP适配、OpenHarness Agent等路由。
- 集成测试套件：覆盖API端点、本体摄入与构建、真实数据一致性、工具/技能注册与执行、图数据库与OPA集成、Docker环境连通性等。
- 外部依赖：Neo4j（图存储）、Redis（缓存）、OPA（策略引擎）、MCP（工具与资源桥接）。
...
## 架构总览
下图展示ODAP平台在集成测试中的关键交互：前端通过HTTP访问后端API，后端路由调用业务服务，持久化依赖Neo4j与SQLite，缓存依赖Redis，权限控制依赖OPA，MCP桥接外部工具与资源。
...
### MCP协议集成测试
- 目标：验证MCP服务器配置合并、工具发现与调用、资源读取。
- 设计要点：
  - 加载MCP服务器配置（本地+插件远程），断言合并后的服务器集合。
  - 工具注册到默认工具注册表，输入校验与执行结果断言。
  - 资源列举工具可用，读取资源返回预期内容。
- 关键断言示例（路径参考）：
  - 配置合并与服务器集合
  - 工具注册与调用
...
participant Manager as "FakeMcpManager"
Test->>Settings : 提供本地MCP服务器配置
Test->>Plugin : 提供启用的插件含远程MCP服务器
Test->>Config : 合并配置
Config-->>Test : 返回合并后的服务器字典
Test->>Registry : 创建默认工具注册表
Registry->>Manager : 注册工具与资源
Test->>Registry : 获取工具并执行
Registry-->>Test : 返回执行结果
```

**图表来源**
- [openharness/tests/test_mcp/test_integration.py:35-80](file://openharness/tests/test_mcp/test_integration.py#L35-L80)

**章节来源**
- [openharness/tests/test_mcp/test_integration.py:1-80](file://openharness/tests/test_mcp/test_integration.py#L1-L80)
- [openharness/tests/fixtures/fake_mcp_server.py:1-22](file://openharness/tests/fixtures/fake_mcp_server.py#L1-L22)
- [odap/biz/integration/mcp_adapter/api/routes.py:1-64](file://odap/biz/integration/mcp_adapter/api/routes.py#L1-L64)

### OpenHarness集成测试
- 目标：验证OpenHarness v1/ v2集成初始化、Agent可用性、工具可用性与会话生命周期。
- 设计要点：
  - 应用生命周期内初始化OpenHarness v1与v2，记录可用工具数量或警告。
  - 在测试环境中根据可用性选择性跳过Docker相关连通性测试。
- 关键断言示例（路径参考）：
  - OpenHarness v1初始化日志与工具数量
  - OpenHarness v2初始化超时/失败处理
  - Docker连通性测试（按环境跳过）

**章节来源**
- [odap/web/app.py:68-120](file://odap/web/app.py#L68-L120)
- [tests/integration/test_v2_integration.py:218-236](file://tests/integration/test_v2_integration.py#L218-L236)
...
## 结论
本指南提供了ODAP平台集成测试的系统化实施方法，覆盖API端点、数据摄入与构建、真实数据一致性、工具/技能与审计、图数据库与OPA、MCP协议与OpenHarness集成。通过Docker编排与pytest测试框架，结合Mock与真实外部依赖，能够稳定、高效地验证模块间接口、数据流与系统行为。建议在CI中定期运行集成测试，并结合性能基准测试持续优化系统稳定性与吞吐能力。
...
- Mock服务：
  ...
  - MCP测试使用fake_mcp_server.py作为轻量级stdio服务器。
...
- 执行顺序：
  ...
  - 最后运行MCP与OpenHarness集成测试，确保外部桥接正常。
- 失败排查：
  - 优先检查外部依赖健康状态与网络连通性。
  - 使用更细粒度的测试用例定位问题模块。
- 性能基准：
