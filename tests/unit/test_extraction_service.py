...
class TestExtractFromNL:
    """Tests for ExtractionService.extract_from_nl (async).

    LLM-dependent tests are skipped when OPENAI_API_KEY is not available.
    Focus on orchestration logic (session management) which doesn't need LLM.
    """

    async def test_creates_extraction_session(self, service, ontology_id):
        """NL extraction creates an extraction session record."""
        # We patch SchemaLevelExtractor to avoid LLM dependency
        ...
        with patch(
            "odap.biz.core.ontology.extraction.services.schema_extractor.SchemaLevelExtractor.extract_from_text",
            new_callable=AsyncMock,
            return_value=mock_result,
        ):
            result = await service.extract_from_nl(
                ontology_id=ontology_id,
                text="A product management system with products and categories",
            )
    ...
    async def test_extraction_result_structure(self, service, ontology_id):
        """NL extraction result contains expected keys."""
        from unittest.mock import AsyncMock, patch

        mock_result = {
            "status": "ok",
            "object_types": [_make_object_type(name="product")],
            "link_types": [_make_link_type(name="product_to_category", source_type="product", target_type="category")],
            "action_types": [_make_action_type(name="create_product", target_object_type="product")],
            "rule_types": [],
            "process_types": [],
            "function_types": [],
            "indicator_types": [],
            "summary": {"object_types": 1, "link_types": 1, "action_types": 1},
        }
        ...
        with patch(
            ...
            result = await service.extract_from_nl(
                ontology_id=ontology_id,
                text="A product management system",
            )
        ...
        assert len(result["result"]["link_types"]) == 1

    async def test_auto_search_parameter_passed(self, service, ontology_id):
        """auto_search parameter is recorded in session input_data."""
        from unittest.mock import AsyncMock, patch
        ...
        # Verify auto_search was passed to the extractor
        mock_extract.assert_called_once_with("Test domain", True)

        # Verify session input_data records auto_search
        session = service.ontology_service.get_extraction_session(result["session_id"])
        assert session["input_data"]["auto_search"] is True

    async def test_extraction_failure_sets_session_failed(self, service, ontology_id):
        """When NL extraction fails, session status is set to 'failed'."""
        from unittest.mock import AsyncMock, patch
    ...
    async def test_nl_session_status_reviewing_on_success(self, service, ontology_id):
        """On successful NL extraction, session status is 'reviewing'."""
        from unittest.mock import AsyncMock, patch

        mock_result = {
            "status": "ok",
            "object_types": [_make_object_type(name="widget")],
            "link_types": [],
            "action_types": [],
            "rule_types": [],
            "process_types": [],
