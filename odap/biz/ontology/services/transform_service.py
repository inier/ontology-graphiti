...
class OntologyTransformService:
    ...
    async def _transform_url(
        ...
        """URL 数据转换 - 抓取网页内容"""
        from odap.biz.ontology.ingestion import WebScraper, FreeNewsIngester
    ...
    def _dict_to_ontology_document(self, data: Dict, metadata: Dict) -> 'OntologyDocument':
        ...
        # 创建新文档
        now = datetime.now(timezone.utc).isoformat()
        from odap.biz.ontology.schema.document import DataSource, DocumentMeta
        ...
        # 添加实体
        if "entity" in data:
            from odap.biz.ontology.schema.document import OntologyEntity
