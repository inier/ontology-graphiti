...
class KnowledgeNavigator:
    ...
    def __init__(self, graph_client=None, query_service=None):
        ...
        if self._query_service:
            try:
                qr = self._query_service.execute(
                    ...
                    query=f".topo neighbors(id='{start_id}', direction='{direction}', depth=1)",
                    limit=5,
                )
                if qr.rows:
                    path.extend([n["id"] for n in qr.rows[:3]])
            except Exception:
                pass
        elif self._graph_client:
            try:
    ...
    def get_related_entities(self, entity_id: str, depth: int = 1) -> List[Dict]:
        ...
        if self._query_service:
            try:
                qr = self._query_service.execute(
                    workspace_id="default",
                    query=f".topo neighbors(id='{entity_id}', depth={depth})",
                    limit=20,
                )
                related = qr.rows
            except Exception:
    ...
    def get_entity_context(self, entity_id: str) -> Dict:
        ...
        if self._query_service:
            try:
                neighbors_qr = self._query_service.execute(
                    workspace_id="default",
                    query=f".topo neighbors(id='{entity_id}', depth=1)",
                    limit=20,
                )
                context["neighbors"] = neighbors_qr.rows
                entity_qr = self._query_service.execute(
                    workspace_id="default",
...
class UserCognitionEngine:
    ...
    def __init__(self, graph_client=None, query_service=None):
        ...
        self._knowledge_navigator = KnowledgeNavigator(graph_client, query_service=query_service)
        self._reasoning_tracker = ReasoningPathTracker()
...
def get_cognition_engine(graph_client=None, query_service=None) -> UserCognitionEngine:
    """获取全局用户认知引擎"""
    global _global_cognition_engine
    if _global_cognition_engine is None:
        _global_cognition_engine = UserCognitionEngine(graph_client, query_service=query_service)
