...
def generate_oms_seed_data():
    """
    从 Domain ENTITY_TYPES 生成 OMS 种子数据
    仅在 OMS 首次初始化时调用

    Returns:
        Dict[str, List[Dict]]: 含 object_types 和 action_types 两个列表
    """
    from odap.biz.core.ontology.oms.schemas import (
    ...
    for type_name, type_def in ENTITY_TYPES.items():
        properties = []
        for prop_name, prop_type_str in type_def.get("properties", {}).items():
            prop_type = PropertyType.STRING
            if prop_type_str in ("int", "integer", "float"):
                prop_type = PropertyType.FLOAT
            elif prop_type_str in ("bool", "boolean"):
                prop_type = PropertyType.BOOLEAN
            elif prop_type_str in ("datetime", "timestamp"):
        ...
        links = []
        for rel_name, rel_target in type_def.get("relationships", {}).items():
        ...
        object_types.append(ObjectTypeDefinition(
            type_id=type_name.lower(),
            name=type_name,
    ...
    return {"object_types": object_types, "action_types": []}