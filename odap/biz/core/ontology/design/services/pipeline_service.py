...
# P0-4 fix (R-P0-001): use event bus instead of importing from application/*.
from ..events import get_event_bus, EntityExtractedEvent, OntologyVersionRolledBackEvent
...
"""
            ...
            raise ValueError(LLMFallback.handle_unavailable("LLM", e)["message"])

    def _register_entity_types_from_extraction(self, entities: List):
        # P0-4 fix (R-P0-001): do NOT import OMS directly here. Publish an
        # event and let the application layer (which is allowed to import
        # OMS) handle the registration.
        try:
            # Convert list to tuple for frozen dataclass
            entities_tuple = tuple(
                e if isinstance(e, dict) else {} for e in entities
            )
            bus = get_event_bus()
            handlers_called = bus.publish(
                EntityExtractedEvent(entities=entities_tuple)
            )
            if handlers_called == 0:
                logger.debug(
                    "EntityExtractedEvent: no handlers registered. "
                    "OMS will not auto-register entity types."
                )
            else:
                logger.info(
                    f"EntityExtractedEvent: notified {handlers_called} handler(s)"
                )
        except Exception as e:
...
class OntologyPipeline:
    ...
    async def rollback(self, version_id: str, workspace_id: str = "default", user_id: str = "system") -> OntologyVersion:
        doc = await self.versions.get_doc(version_id)
        if doc is None:
            raise ValueError(f"版本 {version_id} 不存在或快照已丢失")

        # 获取当前版本信息用于操作历史记录
        current_version_id = self.versions.get_latest_version_id() if hasattr(self.versions, 'get_latest_version_id') else None
        ...
        new_version = await self.ingest(doc)
        ...
        try:
            ...
            history.record_operation(
                ...
                resource_id=new_version.ontology_id,
                before_state={"ontology_id": new_version.ontology_id, "version_id": current_version_id},
                after_state={"ontology_id": new_version.ontology_id, "version_id": new_version.version_id},
            )
        except Exception as e:
            logger.warning(f"回滚操作历史记录失败: {e}")
        ...
        asyncio.create_task(self._emit_rollback_hook(rollback_payload))

        return new_version

    async def _emit_rollback_hook(self, payload: dict):
        """发布 ontology.version.rollback 事件，通知关联资产需要更新"""
        try:
            context = HookContext(event_name="ontology.version.rollback")
            context.set_data("payload", payload)

            hooks = self.hooks.get_hooks("ontology.version.rollback", HookPhase.POST)
            for hook in hooks:
                try:
                    result = hook.handler(context, payload)
                    if asyncio.iscoroutine(result):
                    ...
                    logger.error(f"Rollback Hook {hook.name} 执行失败: {e}")
        except Exception as e:
            logger.error(f"Rollback Hook 广播失败: {e}")
    ...
    def register_rollback_hook(self, handler):
        """注册版本回滚 Hook，用于标记关联资产(SemanticMap/BusinessRule/LogicModel/MetricSystem)为 needs_update"""
        self.hooks.register(
            event="ontology.version.rollback",
            name=getattr(handler, "__name__", str(id(handler))),
            handler=handler,
            phase=HookPhase.POST,
            description="版本回滚时标记关联资产需要更新",
        )
...
def get_pipeline_service() -> OntologyPipeline:
    ...
    if _pipeline_instance is None:
        ...
        # 注册默认的版本回滚 Hook：标记关联资产为 needs_update
        _pipeline_instance.register_rollback_hook(_on_ontology_version_rollback)
    return _pipeline_instance


def _on_ontology_version_rollback(context, payload: dict):
    """版本回滚时标记关联资产(SemanticMap/BusinessRule/LogicModel/MetricSystem)为 needs_update

    P0-4 fix (R-P0-001): do NOT import servitization or agent modules here.
    Publish an event and let the application layer handle it.
    """
    try:
        ontology_id = payload.get("ontology_id", "")
        new_version_id = payload.get("version_id", "")
        bus = get_event_bus()
        handlers_called = bus.publish(
        ...
        if handlers_called == 0:
            logger.debug(
                "OntologyVersionRolledBackEvent: no handlers registered. "
                "Servitization and agent cache will not be refreshed."
            )
        else:
            logger.info(
                f"OntologyVersionRolledBackEvent: notified {handlers_called} handler(s) "
                f"for ontology_id={ontology_id}, version_id={new_version_id}"
            )
    except Exception as e:
        logger.warning(f"版本回滚关联资产标记失败: {e}")
