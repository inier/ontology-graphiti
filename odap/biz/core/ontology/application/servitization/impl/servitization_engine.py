...
class KnowledgeServitizationEngine(IKnowledgeServitizationEngine):
    ...
    def _generate_query_skill_code(self, type_name: str, obj_type: Dict) -> str:
        ...
        return (
            ...
            f'        from odap.infra.query import get_query_service\n'
            f'        qs = get_query_service()\n'
            f'        filters = {{k: v for k, v in params.items() if k in {prop_names}}}\n'
            f'        result = qs.execute("default", ".entity with(type=\'{type_name}\') list()", limit=100)\n'
            f'        return {{"results": result.rows, "count": result.total}}\n'
        )
    ...
    def _generate_aggregate_skill_code(self, type_name: str, stat_props: List[Dict]) -> str:
        ...
        return (
            ...
            f'        from odap.infra.query import get_query_service\n'
            f'        qs = get_query_service()\n'
            f'        result = qs.execute("default", ".entity with(type=\'{type_name}\') list()", limit=1000)\n'
            f'        nodes = result.rows\n'
