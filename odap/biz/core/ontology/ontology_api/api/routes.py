...
- 类型定义写入操作代理到 TypeRegistry 统一入口，确保 OMS 缓存同步
- 所有路由操作均记录审计日志
"""
...
_REGISTRY_MSG = "建议使用 /api/ontology/registry/object-types 统一入口，确保 OMS 缓存同步"
...
async def list_ontologies(
    ...
    try:
        ...
        _audit("ontology_list_failed", uid, "failure", str(e),
...
async def get_ontology(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_get_failed", uid, "failure", result.get("message", ""),
        ...
        _audit("ontology_get_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_ontology(data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_create_failed", uid, "failure", result.get("message", ""),
                   details={"name": data.get("name")}, workspace_id=ws_id)
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_create", uid, "success", "Ontology created",
        ...
        _audit("ontology_create_failed", uid, "failure", str(e),
               details={"name": data.get("name")}, workspace_id=ws_id)
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_ontology(ontology_id: str, data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_update_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_update", uid, "success", "Ontology updated",
               details={"ontology_id": ontology_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_update_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_ontology(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_delete_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_delete", uid, "success", "Ontology deleted",
               details={"ontology_id": ontology_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_delete_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def commit_schema_version(ontology_id: str, data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_commit_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_commit", uid, "success", "Schema version committed",
               details={"ontology_id": ontology_id, "version_id": result.get("version_id")})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_commit_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_schema_versions(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_list_versions_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def diff_schema_versions(
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_diff_failed", uid, "failure", result.get("message", ""),
        ...
        _audit("ontology_diff_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "version_id_a": version_id_a, "version_id_b": version_id_b})
        raise HTTPException(status_code=500, detail=str(e))
...
async def rollback_schema_version(ontology_id: str, data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_rollback_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "target_version_id": target_version_id})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_rollback", uid, "success", "Schema version rolled back",
               details={"ontology_id": ontology_id, "target_version_id": target_version_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_rollback_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "target_version_id": target_version_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_object_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_object_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_object_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    uid = _user_id(user)
    try:
        # 通过 TypeRegistry 统一写入，确保 OMS 缓存同步
        registry = _get_type_registry()
        result = registry.create_object_type(ontology_id, data)
        if result.get("status") == "error":
            _audit("ontology_object_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_object_type_create", uid, "success", "Object type created",
        ...
        _audit("ontology_object_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_object_type(type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    uid = _user_id(user)
    try:
        # 通过 TypeRegistry 统一更新，确保 OMS 缓存同步
        registry = _get_type_registry()
        result = registry.update_object_type(type_id, data)
        if result.get("status") == "error":
            _audit("ontology_object_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_object_type_update", uid, "success", "Object type updated",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_object_type_update_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_object_type(type_id: str, response: Response, user=Depends(get_current_user)):
    uid = _user_id(user)
    try:
        # 通过 TypeRegistry 统一删除，确保 OMS 缓存同步
        registry = _get_type_registry()
        result = registry.delete_object_type(type_id)
        if result.get("status") == "error":
            _audit("ontology_object_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_object_type_delete", uid, "success", "Object type deleted",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_object_type_delete_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_link_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_link_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_link_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_link_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_link_type_create", uid, "success", "Link type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_link_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_link_type(link_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_link_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"link_id": link_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_link_type_update", uid, "success", "Link type updated",
               details={"link_id": link_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_link_type_update_failed", uid, "failure", str(e),
               details={"link_id": link_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_link_type(link_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_link_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"link_id": link_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_link_type_delete", uid, "success", "Link type deleted",
               details={"link_id": link_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_link_type_delete_failed", uid, "failure", str(e),
               details={"link_id": link_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_action_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_action_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_action_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_action_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_action_type_create", uid, "success", "Action type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_action_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_action_type(action_type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_action_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"action_type_id": action_type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_action_type_update", uid, "success", "Action type updated",
               details={"action_type_id": action_type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_action_type_update_failed", uid, "failure", str(e),
               details={"action_type_id": action_type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_action_type(action_type_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_action_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"action_type_id": action_type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_action_type_delete", uid, "success", "Action type deleted",
               details={"action_type_id": action_type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_action_type_delete_failed", uid, "failure", str(e),
               details={"action_type_id": action_type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_process_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_process_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_process_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_process_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_process_type_create", uid, "success", "Process type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_process_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_process_type(type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_process_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_process_type_update", uid, "success", "Process type updated",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_process_type_update_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_process_type(type_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_process_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_process_type_delete", uid, "success", "Process type deleted",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_process_type_delete_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_rule_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_rule_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_rule_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_rule_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_rule_type_create", uid, "success", "Rule type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_rule_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_rule_type(type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_rule_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_rule_type_update", uid, "success", "Rule type updated",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_rule_type_update_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_rule_type(type_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_rule_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_rule_type_delete", uid, "success", "Rule type deleted",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_rule_type_delete_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_function_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_function_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_function_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_function_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_function_type_create", uid, "success", "Function type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_function_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_function_type(type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_function_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_function_type_update", uid, "success", "Function type updated",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_function_type_update_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_function_type(type_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_function_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_function_type_delete", uid, "success", "Function type deleted",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_function_type_delete_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_indicator_types(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        _audit("ontology_indicator_type_list_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_indicator_type(ontology_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_indicator_type_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "name": data.get("name")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_indicator_type_create", uid, "success", "Indicator type created",
               details={"ontology_id": ontology_id, "type_id": result.get("type_id"), "name": data.get("name")})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_indicator_type_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "name": data.get("name")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_indicator_type(type_id: str, data: dict, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_indicator_type_update_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_indicator_type_update", uid, "success", "Indicator type updated",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_indicator_type_update_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_indicator_type(type_id: str, response: Response, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_indicator_type_delete_failed", uid, "failure", result.get("message", ""),
                   details={"type_id": type_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_indicator_type_delete", uid, "success", "Indicator type deleted",
               details={"type_id": type_id})
        response.headers[_REGISTRY_HINT] = _REGISTRY_MSG
        return result
    except HTTPException:
        ...
        _audit("ontology_indicator_type_delete_failed", uid, "failure", str(e),
               details={"type_id": type_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_ontology_graph(ontology_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_graph_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_graph", uid, "success", "Ontology graph retrieved",
               details={"ontology_id": ontology_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_graph_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_database_connections(
    ...
    try:
        ...
        _audit("ontology_db_connection_list_failed", uid, "failure", str(e),
               details={"workspace_id": workspace_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def save_database_connection(data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_db_connection_save_failed", uid, "failure", result.get("message", ""),
                   details={"name": data.get("name")}, workspace_id=ws_id)
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_db_connection_save", uid, "success", "Database connection saved",
        ...
        _audit("ontology_db_connection_save_failed", uid, "failure", str(e),
               details={"name": data.get("name")}, workspace_id=ws_id)
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_database_connection(connection_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_db_connection_delete_failed", uid, "failure", result.get("message", ""),
                   details={"connection_id": connection_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_db_connection_delete", uid, "success", "Database connection deleted",
               details={"connection_id": connection_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_db_connection_delete_failed", uid, "failure", str(e),
               details={"connection_id": connection_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_extraction_session(ontology_id: str, data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_extraction_create_failed", uid, "failure", result.get("message", ""),
                   details={"ontology_id": ontology_id, "extraction_type": data.get("extraction_type")})
            raise HTTPException(status_code=400, detail=result.get("message"))
        _audit("ontology_extraction_create", uid, "success", "Extraction session created",
        ...
        _audit("ontology_extraction_create_failed", uid, "failure", str(e),
               details={"ontology_id": ontology_id, "extraction_type": data.get("extraction_type")})
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_extraction_session(session_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_extraction_get_failed", uid, "failure", result.get("message", ""),
        ...
        _audit("ontology_extraction_get_failed", uid, "failure", str(e),
               details={"session_id": session_id})
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_extraction_session(session_id: str, data: dict, user=Depends(get_current_user)):
    ...
    try:
        ...
        if result.get("status") == "error":
            _audit("ontology_extraction_update_failed", uid, "failure", result.get("message", ""),
                   details={"session_id": session_id})
            raise HTTPException(status_code=404, detail=result.get("message"))
        _audit("ontology_extraction_update", uid, "success", "Extraction session updated",
               details={"session_id": session_id})
        return result
    except HTTPException:
        raise
    except Exception as e:
        _audit("ontology_extraction_update_failed", uid, "failure", str(e),
               details={"session_id": session_id})
        raise HTTPException(status_code=500, detail=str(e))
