...
def get_compose_cmd(*args, dev=False):
    ...
    if dev:
        ...
        if compose_file == COMPOSE_FILE:
            files = [COMPOSE_FILE, COMPOSE_OVERRIDE]
...
def _ensure_registries_config():
    ...
    compose_dir = os.path.join(PROJECT_ROOT, "docker")
    external_refs = []
...
def build_base_images(force=False):
    ...
    for item in BASE_BUILD_IMAGES:
        ...
        rc = _build_image(tag, os.path.join(PROJECT_ROOT, item["dockerfile"]),
                          os.path.join(PROJECT_ROOT, item["context"]))
        if rc == 0:
            ok(f"{tag} 构建成功")
        else:
...
def cmd_rebuild(target=""):
    ...
    for i, t in enumerate(targets, 1):
        ...
        rc = _build_image(
            info["tag"],
            os.path.join(PROJECT_ROOT, info["dockerfile"]),
            os.path.join(PROJECT_ROOT, info["context"]),
        )
        if rc != 0:
