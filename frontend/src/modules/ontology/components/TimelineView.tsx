          ...
          {events.map((event, index) => (
            ...
            >
              ...
              >
                ...
                <div style={{ color: '#8c8c8c', marginTop: 4 }}>{event.description.substring(0, 10)}...</div>
              </div>
            </div>
          ))}
        </div>
          ...
          >
            <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
              <span style={{ padding: '2px 8px', background: '#ff4d4f', color: '#fff', borderRadius: 4, fontSize: 12 }}>
                {events[currentIndex].event_type}
              </span>
              <span style={{ color: '#8c8c8c' }}>{events[currentIndex].location}</span>
            </div>
            <h3 style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>{events[currentIndex].description.substring(0, 30)}</h3>
