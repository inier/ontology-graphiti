...
const CLASSIFICATION_OPTIONS = [
  ...
  { label: 'TS - 绝密', value: 'TS' }, { label: 'S - 机密', value: 'S' },

  { label: 'C - 秘密', value: 'C' }, { label: 'U - 公开', value: 'U' },
...
const LINK_TYPE_OPTIONS = [
  ...
  { label: '关联', value: 'ASSOCIATION' }, { label: '组合', value: 'COMPOSITION' },

  { label: '依赖', value: 'DEPENDENCY' }, { label: '继承', value: 'INHERITANCE' },
...
function TypeDefList<T extends { name: string; display_name?: string; description?: string }>({
  ...
  items, selectedId, onSelect, onDelete, getId, extra, searchPlaceholder = '搜索...',
  ...
  return (
    ...
    <Card title={null} size="small" style={{ height: '100%' }} styles={{ body: { padding: 0 } }}>
      ...
      <div style={{ padding: '8px 12px' }}>
        ...
        <Input prefix={<SearchOutlined />} placeholder={searchPlaceholder} value={search}
...
function ObjectTypeEditor({ item, objectTypes, onSave }: ObjectTypeEditorProps) {
  ...
  if (!item) return <Card><Form form={form} layout="vertical"><Empty description="请选择一个对象类型" /></Form></Card>;
  ...
  return (
    ...
    <Card title={item.display_name || item.name} extra={<AntButton type="primary" onClick={handleSave}>保存</AntButton>}
      ...
      <Form form={form} layout="vertical">
        ...
        <Row gutter={16}>
          ...
          <Col span={8}><Form.Item name="name" label="名称" rules={[{ required: true, message: '必填' }]}><Input /></Form.Item></Col>

          <Col span={8}><Form.Item name="display_name" label="显示名称"><Input /></Form.Item></Col>

          <Col span={8}><Form.Item name="classification_level" label="密级" initialValue="U"><AntSelect options={CLASSIFICATION_OPTIONS} /></Form.Item></Col>
      ...
      <Divider titlePlacement="left">属性列表</Divider>

      {properties.map((prop, idx) => (

        <Row key={idx} gutter={8} style={{ marginBottom: 8 }} align="middle">

          <Col span={5}><Input placeholder="属性名" value={prop.name as string} onChange={(e) => updProp(idx, 'name', e.target.value)} size="small" /></Col>

          <Col span={4}><AntSelect placeholder="类型" value={prop.property_type as string} onChange={(v) => updProp(idx, 'property_type', v)} options={PROPERTY_TYPE_OPTIONS} size="small" style={{ width: '100%' }} /></Col>

          <Col span={3}><AntSelect placeholder="必填" value={prop.required ? 'true' : 'false'} onChange={(v) => updProp(idx, 'required', v === 'true')} options={[{ label: '必填', value: 'true' }, { label: '可选', value: 'false' }]} size="small" style={{ width: '100%' }} /></Col>

          <Col span={5}><Input placeholder="默认值" value={(prop.default_value as string) || ''} onChange={(e) => updProp(idx, 'default_value', e.target.value)} size="small" /></Col>
      ...
      <AntButton type="dashed" icon={<PlusOutlined />} onClick={addProp} size="small" style={{ width: '100%' }}>新增属性</AntButton>

      <Divider titlePlacement="left">关系列表</Divider>

      {links.map((link, idx) => (

        <Row key={idx} gutter={8} style={{ marginBottom: 8 }} align="middle">

          <Col span={4}><Input placeholder="关系名" value={link.name as string} onChange={(e) => updLink(idx, 'name', e.target.value)} size="small" /></Col>

          <Col span={5}><AntSelect placeholder="目标类型" value={link.target_type as string || undefined} onChange={(v) => updLink(idx, 'target_type', v)} options={OBJ_TYPE_OPTS(objectTypes)} size="small" style={{ width: '100%' }} /></Col>

          <Col span={4}><AntSelect placeholder="基数" value={link.cardinality as string} onChange={(v) => updLink(idx, 'cardinality', v)} options={CARDINALITY_OPTIONS} size="small" style={{ width: '100%' }} /></Col>

          <Col span={4}><AntSelect placeholder="关系类型" value={link.link_type as string} onChange={(v) => updLink(idx, 'link_type', v)} options={LINK_TYPE_OPTIONS} size="small" style={{ width: '100%' }} /></Col>
      ...
      <AntButton type="dashed" icon={<PlusOutlined />} onClick={addLink} size="small" style={{ width: '100%' }}>新增关系</AntButton>
...
function SimpleTypeDefEditor({ item, getId, title, extraFields, onSave }: SimpleTypeDefEditorProps) {
  ...
  if (!item) return <Card><Form form={form} layout="vertical"><Empty description={`请选择一个${title}`} /></Form></Card>;
  ...
  return (
    ...
    <Card title={`${title}: ${item.display_name || item.name}`} extra={<AntButton type="primary" onClick={handleSave}>保存</AntButton>}
      ...
      <Form form={form} layout="vertical">
        ...
        <Row gutter={16}>
          ...
          <Col span={12}><Form.Item name="name" label="名称" rules={[{ required: true, message: '必填' }]}><Input /></Form.Item></Col>
...
export function OntologyDesignerPage() {
  ...
  const handleDiff = useCallback(async (versionIdA: string, versionIdB: string) => {
    ...
    try {
      ...
      message.error(`版本对比失败: ${(e as Error).message}`);
    ...
    <div style={{ height: 'calc(100vh - 200px)', overflow: 'hidden' }}>
      ...
      <Row gutter={16} style={{ height: '100%' }}>
        ...
        <Col span={6} style={{ height: '100%', overflow: 'auto' }}>
          ...
          <div style={{ marginBottom: 8 }}><AntButton type="primary" icon={<PlusOutlined />} block size="small" onClick={() => handleCreate(tab)} data-tour="ontology-add-type-btn">新增</AntButton></div>
  ...
  if (!currentOntology) {
    ...
    return (
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <PageHeader title="本体设计器" actions={

          <Space>

            <AntButton icon={<SwapOutlined />} onClick={() => { setSelectorCreate(false); setSelectorOpen(true); }}>切换本体</AntButton>

            <AntButton type="primary" icon={<PlusOutlined />} onClick={() => { setSelectorCreate(true); setSelectorOpen(true); }}>新建本体</AntButton>
  ...
  if (!designMethod) {
    ...
    return (
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <PageHeader title="本体设计器" actions={<AntButton icon={<SwapOutlined />} onClick={() => setSelectorOpen(true)}>切换本体</AntButton>} />
    ...
    return (
      ...
      <>
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <PageHeader title="本体设计器 — 数据库抽取" actions={<AntButton icon={<SwapOutlined />} onClick={switchOntology}>切换本体</AntButton>} />
    ...
    return (
      ...
      <>
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <PageHeader title="本体设计器 — 自然语言提取" actions={<AntButton icon={<SwapOutlined />} onClick={switchOntology}>切换本体</AntButton>} />
  ...
  return (
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <PageHeader title={`本体设计器 — ${currentOntology.name}`}

        actions={

          <>

            <Tag color="blue">{currentOntology.status === 'ACTIVE' ? 'active' : 'draft'}</Tag>

            <AntButton icon={<DatabaseOutlined />} onClick={() => setDesignMethod('database')}>数据库抽取</AntButton>

            <AntButton icon={<MessageOutlined />} onClick={() => setDesignMethod('natural_language')}>自然语言提取</AntButton>

            <AntButton icon={<HistoryOutlined />} onClick={() => setVersionDrawerOpen(true)}>版本历史</AntButton>

            <AntButton icon={<SwapOutlined />} onClick={switchOntology}>切换本体</AntButton>
            ...
            <AntButton type="primary" icon={<CheckCircleOutlined />} onClick={() => setCommitModalOpen(true)}>提交版本</AntButton>
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab} style={{ flex: 1, overflow: 'hidden' }} items={[
        ...
        { key: 'object', label: '对象类型', children: twoPanel('object', objectTypes, <ObjectTypeEditor item={selObj} objectTypes={objectTypes} onSave={(id, d) => handleUpdate('object', id, d)} />, (t: ObjectTypeDefinition) => t.type_id) },

        { key: 'link', label: '关系类型', children: twoPanel('link', linkTypes,

          <SimpleTypeDefEditor item={selLink} getId={(i) => (i as LinkTypeDefinition).link_id} title="关系类型" extraFields={<>

            <Row gutter={16}>

              <Col span={12}><Form.Item name="source_type" label="源对象类型"><AntSelect options={objOpts} /></Form.Item></Col>

              <Col span={12}><Form.Item name="target_type" label="目标对象类型"><AntSelect options={objOpts} /></Form.Item></Col>

            </Row>

            <Row gutter={16}>

              <Col span={12}><Form.Item name="cardinality" label="基数"><AntSelect options={CARDINALITY_OPTIONS} /></Form.Item></Col>

              <Col span={12}><Form.Item name="link_type" label="关系类型"><AntSelect options={LINK_TYPE_OPTIONS} /></Form.Item></Col>

            </Row>

          </>} onSave={(id, d) => handleUpdate('link', id, d)} />, (t: LinkTypeDefinition) => t.link_id) },

        { key: 'action', label: '动作类型', children: twoPanel('action', actionTypes,

          <SimpleTypeDefEditor item={selAction} getId={(i) => (i as ActionTypeDefinition).action_type_id} title="动作类型" extraFields={

            <Row gutter={16}>

              <Col span={12}><Form.Item name="target_object_type" label="目标对象类型"><AntSelect options={objOpts} /></Form.Item></Col>

              <Col span={12}><Form.Item name="confirmation_required" label="需要确认" initialValue={true}><AntSelect options={[{ label: '是', value: true }, { label: '否', value: false }]} /></Form.Item></Col>

            </Row>

          } onSave={(id, d) => handleUpdate('action', id, d)} />, (t: ActionTypeDefinition) => t.action_type_id) },

        { key: 'process', label: '业务过程', children: twoPanel('process', processTypes,

          <SimpleTypeDefEditor item={selProcess} getId={(i) => (i as ProcessTypeDefinition).process_type_id} title="业务过程" extraFields={

            <Form.Item name="related_object_types" label="关联对象类型"><AntSelect mode="multiple" options={objOpts} /></Form.Item>

          } onSave={(id, d) => handleUpdate('process', id, d)} />, (t: ProcessTypeDefinition) => t.process_type_id) },

        { key: 'rule', label: '规则', children: twoPanel('rule', ruleTypes,

          <SimpleTypeDefEditor item={selRule} getId={(i) => (i as RuleTypeDefinition).rule_type_id} title="规则" extraFields={<>

            <Row gutter={16}>

              <Col span={12}><Form.Item name="condition" label="触发条件"><Input.TextArea rows={2} /></Form.Item></Col>

              <Col span={12}><Form.Item name="action" label="约束动作"><Input.TextArea rows={2} /></Form.Item></Col>
        ...
        { key: 'function', label: '逻辑函数', children: twoPanel('function', functionTypes,

          <SimpleTypeDefEditor item={selFunc} getId={(i) => (i as FunctionTypeDefinition).function_type_id} title="逻辑函数" extraFields={<>

            <Row gutter={16}>

              <Col span={12}><Form.Item name="logic_types" label="逻辑类型"><AntSelect mode="multiple" options={['filter', 'transform', 'validate', 'compute'].map((t) => ({ label: t, value: t }))} /></Form.Item></Col>

              <Col span={12}><Form.Item name="return_type" label="返回类型"><AntSelect options={PROPERTY_TYPE_OPTIONS} /></Form.Item></Col>
        ...
        { key: 'indicator', label: '指标', children: twoPanel('indicator', indicatorTypes,

          <SimpleTypeDefEditor item={selInd} getId={(i) => (i as IndicatorTypeDefinition).indicator_type_id} title="指标" extraFields={<>

            <Row gutter={16}>

              <Col span={8}><Form.Item name="indicator_types" label="指标类型"><AntSelect mode="multiple" options={['kpi', 'metric', 'dimension'].map((t) => ({ label: t, value: t }))} /></Form.Item></Col>

              <Col span={8}><Form.Item name="formula" label="计算公式"><Input /></Form.Item></Col>
        ...
        { key: 'graph', label: '图谱', icon: <ApartmentOutlined />, children: (
          ...
          <div>
            ...
            {graphNodes.length > 0 ? (
              ...
              : <Card style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Empty description="暂无图谱数据，请先定义对象类型和关系类型" /></Card>}
      ...
      <Modal title="提交版本" open={commitModalOpen} onOk={handleCommit} onCancel={() => { setCommitModalOpen(false); commitForm.resetFields(); }} okText="提交" cancelText="取消">

        <Form form={commitForm} layout="vertical">

          <Form.Item name="changelog" label="变更日志" rules={[{ required: true, message: '请输入变更说明' }]}>

            <Input.TextArea rows={4} placeholder="描述本次版本变更内容..." />
