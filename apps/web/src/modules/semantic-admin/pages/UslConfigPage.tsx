    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <div>
        <Typography.Title level={5} style={{ marginTop: 0, marginBottom: 12 }}>
          {t('不相交约束对（Disjoint Pairs）')}
        </Typography.Title>
        <DisjointPairTable />
      </div>
      <div>
        <Typography.Title level={5} style={{ marginTop: 0, marginBottom: 12 }}>
          {t('关系基数约束（Cardinality）')}
        </Typography.Title>
        <CardinalityTable />
      </div>
    </Space>
...
export function UslConfigPage() {
  ...
  /** USL 配置页 5 子 Tab */
  const uslSubTabItems: TabsProps['items'] = useMemo(() => [
    {
      key: 'domains',
      label: (
        <span>
    ...
    if (needDomainContext && !currentDomain) {
      return (
        <Alert
          type="info"
          showIcon
          message={t('请先在「语义域列表」Tab 中选中一个语义域')}
          description={
            <Text type="secondary">
              {t('术语、层级、属性、约束都从属于特定语义域；点击「语义域列表」表格行的「选择」按钮建立上下文后再切回本 Tab。')}
            </Text>
          }
        />
      );
