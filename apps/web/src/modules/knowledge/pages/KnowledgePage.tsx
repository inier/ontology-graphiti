...
export function KnowledgePage() {
  ...
  const handleAddSynonym = async () => {
    if (!synonymCanonical || !synonymValue) {
      message.warning(t('请输入规范词和同义词'));
      return;
    }
    await addSynonym(synonymCanonical, synonymValue);
    setSynonymCanonical('');
    setSynonymValue('');
    message.success(t('同义词已添加'));
  };

  const handleAddExpansionRule = async () => {
    if (!rulePattern || !ruleExpansion) {
      message.warning(t('请输入模式和扩写'));
      return;
    }
    await addExpansionRule(rulePattern, ruleExpansion);
    setRulePattern('');
    setRuleExpansion('');
    message.success(t('扩写规则已添加'));
  };

  const synonymColumns = [
    { title: t('规范词'), dataIndex: 'canonical', key: 'canonical' },
    { title: t('同义词'), dataIndex: 'synonyms', key: 'synonyms', render: (syns: string[]) => syns?.map((s: string) => <Tag key={s}>{s}</Tag>) },
  ];
  ...
  const expansionColumns = [
    { title: t('模式'), dataIndex: 'pattern', key: 'pattern' },
    { title: t('扩写'), dataIndex: 'expansion', key: 'expansion', render: (exp: string[]) => exp?.map((e: string) => <Tag key={e}>{e}</Tag>) },
  ];

  return (
    <div>
      <Card title={t('知识导航与语义层配置')}>
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          {
            key: 'navigation',
            label: <span><CompassOutlined /> {t('知识导航')}</span>,
            children: (
              <Space orientation="vertical" style={{ width: '100%' }} size="middle">
                <Space>
                  <Input
                    placeholder={t('输入实体ID')}
                    value={navEntityId}
                  ...
                  >
                    <option value="outbound">{t('出向')}</option>
                    <option value="inbound">{t('入向')}</option>
                    <option value="both">{t('双向')}</option>
                  </select>
                ...
                {navigationResults && (
                  <Descriptions title={t('导航结果')} column={1}>
                    <Descriptions.Item label={t('实体ID')}>{navigationResults.entity_id}</Descriptions.Item>
                    <Descriptions.Item label={t('导航路径')}>
                      {navigationResults.navigation_path?.map((p: string, i: number) => (
                        <Tag key={i}>{p}</Tag>
                      ))}
                    </Descriptions.Item>
                    <Descriptions.Item label={t('相关实体数')}>{navigationResults.related_entities?.length || 0}</Descriptions.Item>
                  </Descriptions>
                )}
              </Space>
            ),
          ...
          {
            key: 'intent',
            label: <span><BulbOutlined /> {t('意图解析')}</span>,
            children: (
              <Space orientation="vertical" style={{ width: '100%' }} size="middle">
                <TextArea
                  placeholder={t('输入自然语言查询...')}
                  value={intentInput}
                ...
                {intentResult && (
                  <Descriptions title={t('解析结果')} column={2}>
                    <Descriptions.Item label={t('意图')}>{String(intentResult.intent || '')}</Descriptions.Item>
                    <Descriptions.Item label={t('置信度')}>{String(intentResult.confidence || '')}</Descriptions.Item>
                    <Descriptions.Item label={t('实体')} span={2}>
                      {(intentResult.entities as string[])?.map((e: string, i: number) => <Tag key={i}>{e}</Tag>)}
                    </Descriptions.Item>
                  </Descriptions>
                )}
                {taskPlan && (
                  <Card title={t('任务规划')} size="small">
                    <List
                      size="small"
          ...
          {
            key: 'semantic',
            label: <span><ToolOutlined /> {t('语义配置')}</span>,
            children: (
              <Space orientation="vertical" style={{ width: '100%' }} size="middle">
                <Card title={t('同义词映射')} size="small">
                  <Space style={{ marginBottom: 16 }}>
                    <Input placeholder={t('规范词')} value={synonymCanonical} onChange={(e) => setSynonymCanonical(e.target.value)} style={{ width: 150 }} />
                    <Input placeholder={t('同义词')} value={synonymValue} onChange={(e) => setSynonymValue(e.target.value)} style={{ width: 150 }} />
                    <Button type="primary" onClick={handleAddSynonym}>{t('添加')}</Button>
                    <Button onClick={() => loadSynonyms()}>{t('刷新')}</Button>
                  </Space>
                  <AdvancedTable columns={synonymColumns} dataSource={synonymData} size="small" pagination={false} />
                </Card>
                <Card title={t('扩写规则')} size="small">
                  <Space style={{ marginBottom: 16 }}>
                    <Input placeholder={t('模式')} value={rulePattern} onChange={(e) => setRulePattern(e.target.value)} style={{ width: 150 }} />
                    <Input placeholder={t('扩写')} value={ruleExpansion} onChange={(e) => setRuleExpansion(e.target.value)} style={{ width: 150 }} />
                    <Button type="primary" onClick={handleAddExpansionRule}>{t('添加')}</Button>
                    <Button onClick={() => loadExpansionRules()}>{t('刷新')}</Button>
                  </Space>
