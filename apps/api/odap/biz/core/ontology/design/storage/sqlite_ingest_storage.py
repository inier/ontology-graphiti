...
class SQLiteIngestStorage:
    ...
    def _init_db(self):
        ...
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_validation_issues_ontology
            ON validation_issues(ontology_id)
        ''')
        ...
        for doc in docs:
            for rel in doc.get("relations", []):
                ...
                if source and target:
                    links.append({
                        "id": rel.get("relation_id", f"rel-{source[:8]}-{target[:8]}"),
                        "source": source,
                        "target": target,
    ...
    def save_ontology_document(self, doc: OntologyDocument) -> str:
        ...
        if doc_data.get('scenario_id'):
            cursor.execute('''
                INSERT OR REPLACE INTO scenario_documents
                (scenario_id, doc_id, meta, entities, events, relations, ontology_version, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                doc_data.get('scenario_id'),
    ...
    def get_validation_rule(self, rule_id: str) -> Optional[Dict[str, Any]]:
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM validation_rules WHERE rule_id = ?', (rule_id,))
        row = cursor.fetchone()
        conn.close()
        if not row:
            return None
    ...
    def list_validation_rules(self, filters: Dict[str, Any] = None,
        ...
        sql = f'SELECT * FROM validation_rules{where} ORDER BY created_at DESC LIMIT ? OFFSET ?'
    ...
    def count_validation_rules(self, filters: Dict[str, Any] = None) -> int:
        conn = self._get_conn()
        cursor = conn.cursor()
        conditions = []
        params = []
        if filters:
            ...
            if filters.get('severity'):
                ...
                params.append(filters['severity'])
        where = (' WHERE ' + ' AND '.join(conditions)) if conditions else ''
        cursor.execute(f'SELECT COUNT(*) FROM validation_rules{where}', params)
        count = cursor.fetchone()[0]
        conn.close()
        return count

    def save_validation_issue(self, issue: Dict[str, Any]) -> str:
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT OR REPLACE INTO validation_issues
            (issue_id, rule_id, rule_name, severity, ontology_id, entity_id,
             relation_id, property_name, message, details, status, auto_fixable,
        ...
        ''', (
            ...
            issue.get('rule_name'),
            issue.get('severity'),
            issue.get('ontology_id'),
            issue.get('entity_id'),
            issue.get('relation_id'),
            issue.get('property_name'),
            issue.get('message'),
            self._serialize_json(issue.get('details')),
    ...
    def get_validation_issue(self, issue_id: str) -> Optional[Dict[str, Any]]:
        conn = self._get_conn()
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM validation_issues WHERE issue_id = ?', (issue_id,))
        row = cursor.fetchone()
        conn.close()
        if not row:
            return None
        d = dict(row)
        d['details'] = self._deserialize_json(d.get('details'))
    ...
    def list_validation_issues(self, ontology_id: str,
        ...
        cursor.execute(f'SELECT * FROM validation_issues{where} ORDER BY timestamp DESC', params)
        rows = cursor.fetchall()
        conn.close()
        results = []
        for row in rows:
    ...
    def update_validation_issue(self, issue_id: str, updates: Dict[str, Any]) -> bool:
        conn = self._get_conn()
        cursor = conn.cursor()
        sets = []
        vals = []
        for k in ('status', 'message', 'details', 'auto_fixable', 'entity_id', 'relation_id', 'property_name'):
            if k in updates:
                if k == 'details':
                    sets.append(f'{k} = ?')
                    vals.append(self._serialize_json(updates[k]))
                elif k == 'auto_fixable':
                    sets.append(f'{k} = ?')
        ...
        cursor.execute(f'UPDATE validation_issues SET {", ".join(sets)} WHERE issue_id = ?', vals)
        affected = cursor.rowcount
        conn.commit()
        conn.close()
        return affected > 0

    def update_entity_name(self, canonical_id: str, new_name: str) -> bool:
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute('UPDATE entity_registry SET name = ? WHERE canonical_id = ?', (new_name, canonical_id))
        affected = cursor.rowcount
        conn.commit()
        conn.close()
        return affected > 0
