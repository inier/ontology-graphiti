...
class SQLiteWarehouse:
    ...
    def save_dataset(self, name: str, description: str = "",
        ...
        try:
            ...
            if existing:
                ...
                conn.execute(
                    "DELETE FROM data_rows WHERE dataset_id = ?", (dataset_id,)
                )
                conn.execute(
                    """UPDATE datasets SET description = ?, schema_json = ?,
                       row_count = ?, updated_at = ? WHERE id = ?""",
                    (description, json.dumps(schema, ensure_ascii=False),
                     len(rows), now, dataset_id),
                )
            else:
                conn.execute(
                    """INSERT INTO datasets (id, name, description, schema_json,
                       row_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (dataset_id, name, description, json.dumps(schema, ensure_ascii=False),
                     len(rows), now, now),
                )
            ...
            for row in rows:
                ...
                conn.execute(
                    "INSERT INTO data_rows (id, dataset_id, row_data, created_at) VALUES (?, ?, ?, ?)",
                    (row_id, dataset_id, json.dumps(row, ensure_ascii=False), now),
                )
            conn.commit()
        finally:
            conn.close()
    ...
    def query(self, dataset_name: str, filters: Optional[Dict[str, Any]] = None,
              limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        conn = sqlite3.connect(self.db_path)
        try:
            dataset = conn.execute(
                "SELECT id, name, schema_json, row_count FROM datasets WHERE name = ?",
                (dataset_name,),
            ).fetchone()
            if not dataset:
                return {"status": "error", "message": f"Dataset '{dataset_name}' not found"}

            dataset_id, name, schema_json, row_count = dataset
            cursor = conn.execute(
                "SELECT row_data FROM data_rows WHERE dataset_id = ?",
                (dataset_id,),
            )
            all_rows = []
            for (row_data_str,) in cursor:
                try:
                    all_rows.append(json.loads(row_data_str))
    ...
    def _apply_filters(self, rows: List[Dict], filters: Dict[str, Any]) -> List[Dict]:
        result = []
        for row in rows:
            match = True
    ...
    def list_datasets(self) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = conn.execute(
                "SELECT id, name, description, schema_json, row_count, created_at, updated_at FROM datasets"
            )
            results = []
            for row in cursor:
                results.append({
    ...
    def get_dataset(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            row = conn.execute(
                "SELECT id, name, description, schema_json, row_count, created_at, updated_at FROM datasets WHERE name = ?",
                (name,),
            ).fetchone()
            if not row:
                return None
    ...
    def load_data(self, table_name: str, records: List[Dict[str, Any]]):
        ...
        if records:
            for key, value in records[0].items():
                schema[key] = type(value).__name__
        self.save_dataset(name=table_name, schema=schema, rows=records)

    def get_table(self, table_name: str) -> List[Dict[str, Any]]:
        result = self.query(table_name, limit=100000)
    ...
    def get_table_schema(self, table_name: str) -> Dict[str, str]:
        ds = self.get_dataset(table_name)
        return ds.get("schema", {}) if ds else {}
    ...
    def create_snapshot(self, name: str, description: str = "") -> DataSnapshot:
        ...
        try:
            ...
            for ds in datasets:
                ds_name = ds["name"]
                rows = self.query(ds_name, limit=100000).get("rows", [])
                data[ds_name] = rows
                entity_counts[ds_name] = len(rows)
        finally:
            conn.close()
        ...
        try:
            conn.execute(
                """INSERT INTO snapshots (snapshot_id, name, description, data_json,
                   entity_counts_json, created_at) VALUES (?, ?, ?, ?, ?, ?)""",
                (snapshot.snapshot_id, name, description,
                 json.dumps(data, ensure_ascii=False),
                 json.dumps(entity_counts, ensure_ascii=False),
    ...
    def get_snapshot(self, snapshot_id: str) -> Optional[DataSnapshot]:
        ...
        try:
            row = conn.execute(
                "SELECT snapshot_id, name, description, data_json, entity_counts_json, created_at FROM snapshots WHERE snapshot_id = ?",
                (snapshot_id,),
            ).fetchone()
            if not row:
                return None
            return DataSnapshot(
                ...
                entity_counts=json.loads(row[4]),
    ...
    def list_snapshots(self) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = conn.execute(
                "SELECT snapshot_id, name, description, entity_counts_json, created_at FROM snapshots"
            )
            return [
                {
                    "snapshot_id": row[0],
                    ...
                    "description": row[2],
                    "created_at": row[4],
                    "entity_counts": json.loads(row[3]),
                }
                for row in cursor
            ]
        finally:
            ...
            for table_name, rows in snapshot.data.items():
                ...
                if rows:
                    for key, value in rows[0].items():
                        schema[key] = type(value).__name__
                self.save_dataset(name=table_name, schema=schema, rows=rows)
        finally:
            conn.close()
...
class QueryService:
    def __init__(self, data_dir: Optional[str] = None):
        try:
            ...
            logger.info("QueryService: using SQLiteWarehouse")
        except Exception as e:
            logger.warning("QueryService: SQLiteWarehouse init failed, falling back to SimulatedWarehouse: %s", e)
